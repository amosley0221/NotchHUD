...
let port: UInt16 = UInt16(ProcessInfo.processInfo.environment["NOTCHHUD_PORT"] ?? "") ?? 8787
...
func send(_ json: String) {
    ...
    if semaphore.wait(timeout: .now() + 3) == .timedOut { failed = true }

    if failed {
        FileHandle.standardError.write(Data("notchhud: could not reach the HUD on 127.0.0.1:\(port)\n".utf8))
        exit(1)
    }
}
